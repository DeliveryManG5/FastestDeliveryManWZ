...
public class AssignDeliveryJob extends javax.swing.JFrame {
    ...
    public static ListInterface<FoodOrder> FoodList = HomePage.list;
    private AttendanceInterface<Attendance> statusList = new LAttendance();
    private ListInterface<FoodOrder> statusList2 = new List();    
    ...
    public AssignDeliveryJob() {
        ...
        for(int i = 1; i <= length2; i++){
            foodOrder = statusList2.get(i);
            jcbxOrderID.addItem(Integer.toString(foodOrder.getOrderID()));
        }    
    }
    ...
    private void jbtnAssignActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jbtnAssignActionPerformed
        ...
        boolean yesno = FoodList.searchRecord(Integer.parseInt(orderid));
        
        if(yesno){
            foodOrder = (FoodOrder)FoodList.getRecord(Integer.parseInt(orderid));
            int orderID = foodOrder.getOrderID();
            String foodName = foodOrder.getFoodName();
            double foodPrice = foodOrder.getFoodPrice();
            int qty = foodOrder.getQty();
            double totalPrice = foodOrder.getTotalPrice();
            String DMID = deliveryManID;
            
            FoodOrder updateFoodOrder = new FoodOrder(orderID, foodName, foodPrice, qty, totalPrice, DMID);
            int position = FoodList.getPosition(Integer.parseInt(orderid));
            FoodList.updateRecord(position, updateFoodOrder);
        }
        //System.exit(0);
        //repaint();
        this.dispose();
